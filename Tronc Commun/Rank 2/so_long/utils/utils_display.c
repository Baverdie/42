...
void	display_player(t_mlx *mlx, int i, int j)
{
	if (mlx->dir_player == 0 || mlx->dir_player == 3)
		mlx_put_image_to_window(mlx->mlx, mlx->window, mlx->img->player_left, j * 64, i * 64);
...
void	display_mob(t_mlx *mlx, int i, int j)
{
	...
	if (!mlx->data->mobs[count]->dir || (mlx->data->mobs[count]->dir == 0 || mlx->data->mobs[count]->dir == 3))
		mlx_put_image_to_window(mlx->mlx, mlx->window, mlx->img->mob_left, j * 64, i * 64);
	else if (mlx->data->mobs[count]->dir == 1)
		mlx_put_image_to_window(mlx->mlx, mlx->window, mlx->img->mob_right, j * 64, i * 64);
